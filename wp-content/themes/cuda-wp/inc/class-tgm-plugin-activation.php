...
if ( ! class_exists( 'Cuda_TGM_Plugin_Activation' ) ) {
	/**
 	 ...
 	 * @author Gary Jones <gamajo@gamajo.com>
 	 */
	class Cuda_TGM_Plugin_Activation {
		...
		/**
	 	 ...
	 	 * @since 1.0.0
	 	 *
	 	 * @var Cuda_TGM_Plugin_Activation
	 	 */
		static $instance;
		 ...
		 * @since 1.0.0
		 *
		 * @see Cuda_TGM_Plugin_Activation::init()
		 */
		 ...
		 * @since 2.0.0
		 *
		 * @see Cuda_TGM_Plugin_Activation::admin_menu()
		 * @see Cuda_TGM_Plugin_Activation::notices()
		 * @see Cuda_TGM_Plugin_Activation::styles()
		 */
		 ...
		 *
		 * @see Cuda_TGM_Plugin_Activation::init()
		 * @see Cuda_TGM_Plugin_Activation::install_plugins_page()
		 */
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php  apply_filters( 'tgmpa_default_screen_icon', 'themes' ); ?>
...
new Cuda_TGM_Plugin_Activation;
...
if ( ! function_exists( 'tgmpa' ) ) {
	...
	function tgmpa( $plugins, $config = array() ) {
		...
		foreach ( $plugins as $plugin )
			Cuda_TGM_Plugin_Activation::$instance->register( $plugin );

		if ( $config )
			Cuda_TGM_Plugin_Activation::$instance->config( $config );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		 ...
		 * and stores it in the global object Cuda_TGM_Plugin_Activation::$instance.
		 *
		 * @since 2.2.0
		 *
		...
		protected function _gather_plugin_data() {
			...
			Cuda_TGM_Plugin_Activation::$instance->admin_init();
			Cuda_TGM_Plugin_Activation::$instance->thickbox();
			...
			foreach ( Cuda_TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				if ( is_plugin_active( $plugin['file_path'] ) )
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', Cuda_TGM_Plugin_Activation::$instance->domain );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', Cuda_TGM_Plugin_Activation::$instance->domain );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', Cuda_TGM_Plugin_Activation::$instance->domain );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', Cuda_TGM_Plugin_Activation::$instance->domain );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', Cuda_TGM_Plugin_Activation::$instance->domain ) : __( 'Recommended', Cuda_TGM_Plugin_Activation::$instance->domain );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', Cuda_TGM_Plugin_Activation::$instance->domain ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', Cuda_TGM_Plugin_Activation::$instance->domain ) );
		 ...
		 * Cuda_TGM_Plugin_Activation class.
		 *
		 * Loops through the registered plugins looking for $name. If it finds it,
		 * it returns the $data from that plugin. Otherwise, returns false.
		 *
		...
		protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
			...
			foreach ( Cuda_TGM_Plugin_Activation::$instance->plugins as $plugin => $values ) {
				if ( $name == $values['name'] && isset( $values[$data] ) )
					return $values[$data];
			}
				...
				$actions = array(
					'install' => sprintf(
						...
						wp_nonce_url(
							add_query_arg(
								array(
									'page'          => Cuda_TGM_Plugin_Activation::$instance->menu,
									'plugin'        => $item['slug'],
									'plugin_name'   => $item['sanitized_plugin'],
									'plugin_source' => $item['url'],
									'tgmpa-install' => 'install-plugin',
								),
								admin_url( Cuda_TGM_Plugin_Activation::$instance->parent_url_slug )
							),
							'tgmpa-install'
						),
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', Cuda_TGM_Plugin_Activation::$instance->domain ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', Cuda_TGM_Plugin_Activation::$instance->domain ),
				'source' => __( 'Source', Cuda_TGM_Plugin_Activation::$instance->domain ),
				'type'   => __( 'Type', Cuda_TGM_Plugin_Activation::$instance->domain ),
				'status' => __( 'Status', Cuda_TGM_Plugin_Activation::$instance->domain )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', Cuda_TGM_Plugin_Activation::$instance->domain ),
				'tgmpa-bulk-activate' => __( 'Activate', Cuda_TGM_Plugin_Activation::$instance->domain ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( is_wp_error( $api ) )
					wp_die( Cuda_TGM_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) );
				...
				$url   = add_query_arg( array( 'page' => Cuda_TGM_Plugin_Activation::$instance->menu ), admin_url( Cuda_TGM_Plugin_Activation::$instance->parent_url_slug ) );
				$nonce = 'bulk-plugins';
				$names = $plugin_names;
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, Cuda_TGM_Plugin_Activation::$instance->domain ), $imploded );
 ...
 * @since 2.2.0
 */
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET[sanitize_key( 'page' )] ) && Cuda_TGM_Plugin_Activation::$instance->menu = $_GET[sanitize_key( 'page' )] ) ) {
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function bulk_install( $packages ) {
				...
				if ( Cuda_TGM_Plugin_Activation::$instance->is_automatic )
			...
			public function run( $options ) {
				...
				if ( Cuda_TGM_Plugin_Activation::$instance->is_automatic ) {
					...
					Cuda_TGM_Plugin_Activation::$instance->populate_file_path();
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', Cuda_TGM_Plugin_Activation::$instance->domain );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', Cuda_TGM_Plugin_Activation::$instance->domain );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', Cuda_TGM_Plugin_Activation::$instance->domain );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', Cuda_TGM_Plugin_Activation::$instance->domain );
				$this->strings['process_failed']      = __( 'Plugin install failed.', Cuda_TGM_Plugin_Activation::$instance->domain );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', Cuda_TGM_Plugin_Activation::$instance->domain );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', Cuda_TGM_Plugin_Activation::$instance->domain );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', Cuda_TGM_Plugin_Activation::$instance->domain );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( Cuda_TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', Cuda_TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', Cuda_TGM_Plugin_Activation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', Cuda_TGM_Plugin_Activation::$instance->domain ) . '</span><span class="hidden">' . __( 'Hide Details', Cuda_TGM_Plugin_Activation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', Cuda_TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', Cuda_TGM_Plugin_Activation::$instance->domain );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', Cuda_TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', Cuda_TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', Cuda_TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', Cuda_TGM_Plugin_Activation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', Cuda_TGM_Plugin_Activation::$instance->domain ) . '</span><span class="hidden">' . __( 'Hide Details', Cuda_TGM_Plugin_Activation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', Cuda_TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', Cuda_TGM_Plugin_Activation::$instance->domain );
				}
			...
			public function bulk_footer() {
				...
				foreach ( Cuda_TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', Cuda_TGM_Plugin_Activation::$instance->menu, admin_url( Cuda_TGM_Plugin_Activation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( Cuda_TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . __( Cuda_TGM_Plugin_Activation::$instance->strings['return'], Cuda_TGM_Plugin_Activation::$instance->domain ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( Cuda_TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', Cuda_TGM_Plugin_Activation::$instance->domain ) . '">' . __( 'Return to the Dashboard', Cuda_TGM_Plugin_Activation::$instance->domain ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
