		...
		class One_Paze_Welcome {
			...
			public function onepaze_activation_admin_notice() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					<div class="notice notice-success is-dismissible">
						<p><?php printf( __( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'one-paze' ), $this->theme_name, admin_url( 'themes.php?page=onepaze-welcome' )  ); ?></p>
						<p><a class="button" href="<?php echo admin_url( 'themes.php?page=onepaze-welcome' ) ?>"><?php _e( 'Lets Get Started', 'one-paze' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function onepaze_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php printf( esc_html__( 'Welcome to %s - Version %s', 'one-paze' ), $this->theme_name, $this->theme_version ); ?></h1>
					<div class="about-text"><?php printf( esc_html__( 'The %s is a Beautifully designed modern WordPress theme. It is one page Parallax layout with ability to configure inner pages as well. It loads super fast, built on customizer and allows you to live preview your changes as you go. Highly configurable and high level of compatibility with major WP plugins.', 'one-paze' ), $this->theme_name ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo admin_url('themes.php?page=onepaze-welcome&section='.$id); ?>" class="<?php echo $nav_class; ?>" >
			...
			public function onepaze_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'onepaze-welcome-screen', 'onepazeWelcomeObject', array(
			...
			public function onepaze_plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'one-paze' ) );
				...
				if (! wp_verify_nonce( $nonce, 'onepaze_plugin_installer_nonce' ))
					wp_die( __( 'Error - unable to verify nonce, please try again.', 'one-paze') );
			...
			public function onepaze_plugin_activation_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'one-paze' ) );
				...
				if (! wp_verify_nonce( $nonce, 'onepaze_plugin_activate_nonce' ))
					die( __( 'Error - unable to verify nonce, please try again.', 'one-paze' ) );
