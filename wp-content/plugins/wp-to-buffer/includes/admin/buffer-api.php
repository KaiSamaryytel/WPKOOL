...
class WP_To_Buffer_Pro_Buffer_API {
    ...
    private function request_wordpress( $url, $method, $params ) {
        ...
        if ( $result['response']['code'] != 200 ) {
            ...
            return new WP_Error( 
                ...
                'Buffer API Error: HTTP Code ' . $result['response']['code'] . '. #' . $body->code . ' - ' . $body->message 
            );
        }
    ...
    private function request_curl( $url, $method, $params ) {
        ...
        if ( $http_code != 200 ) {
            ...
            // Return WP_Error
            return new WP_Error( $http_code, 'Buffer API Error: HTTP Code ' . $http_code . '. #' . $result->code. ' - ' . $result->message );
        }
